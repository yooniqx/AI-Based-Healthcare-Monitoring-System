...
def analyze_dataset(df):
    ...
    if 'selector' in df.columns:
        ...
        if len(selector_dist) > 1:
            ...
            if imbalance_ratio > 2:
                print(f"   âš  Note: Moderate class imbalance detected")
    ...
    if duplicates > 0:
        ...
        print(f"   âš  WARNING: {duplicates} duplicate records found - should be removed during cleaning")
    ...
    print(f"   â„¹ drinks = 0: {zero_drinks} records ({zero_drinks/len(df)*100:.2f}%) - non-drinkers")
